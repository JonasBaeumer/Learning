# Now instead of trying to create every possible string we see that for the current position 
# if the string we are at, if there is a string in our dict that we can use 
# to here to get that substring, if not there is no possible solution and we can stop
# Runtime: O(n * m * s), n length of string, m number of available words, t max length of any word in dict
# Spacetime: O(n), depth of recursive call stack, + cache
